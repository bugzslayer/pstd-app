...
import com.razza.apps.iosched.explore.data.MessageData;
...
import com.razza.apps.iosched.ui.widget.CollectionView;
import com.razza.apps.iosched.ui.widget.CollectionViewCallbacks;
...
public class ExploreIOFragment extends Fragment implements UpdatableView<ExploreModel>,
    ...
    private static final String TAG = LogUtils.makeLogTag(ExploreIOFragment.class);
    ...
    private void updateCollectionView(ExploreModel model) {
        LogUtils.LOGD(TAG, "Updating collection view.");
        ...
        if (keynoteData != null) {
            LogUtils.LOGD(TAG, "Keynote Live stream data found: " + model.getKeynoteData());
        ...
        if (liveStreamData != null && liveStreamData.getSessions().size() > 0) {
            LogUtils.LOGD(TAG, "Live session data found: " + liveStreamData);
            inventoryGroup = new CollectionView.InventoryGroup
        ...
        LogUtils.LOGD(TAG, "Inventory item count:" + inventory.getGroupCount() + " " + inventory
                .getTotalItemCount());
        ...
        for (TopicGroup topic : model.getTopics()) {
            LogUtils.LOGD(TAG, topic.getTitle() + ": " + topic.getSessions().size());
        ...
        for (ThemeGroup theme : model.getThemes()) {
            LogUtils.LOGD(TAG, theme.getTitle() + ": " + theme.getSessions().size());
