...
package com.razza.apps.iosched.videolibrary;
...
import com.razza.apps.iosched.util.ParserUtils;
...
public class VideoLibraryModel implements Model {
    ...
    private static final String TAG = LogUtils.makeLogTag(VideoLibraryModel.class);

    protected static final String KEY_YEAR = "com.razza.apps.iosched.KEY_YEAR";

    protected static final String KEY_TOPIC = "com.razza.apps.iosched.KEY_TOPIC";

    protected static final String KEY_VIDEO_ID = "com.razza.apps.iosched.KEY_VIDEO_ID";
    ...
    public boolean readDataFromCursor(Cursor cursor, QueryEnum query) {
        ...
        if (query == VideoLibraryQueryEnum.VIDEOS) {
            LogUtils.LOGD(TAG, "Reading video library collection Data from cursor.");
            ...
            LogUtils.LOGD(TAG, "Reading my viewed videos Data from cursor.");
            if(cursor.moveToFirst()) {
    ...
    public Loader<Cursor> createCursorLoader(int loaderId, Uri uri, @Nullable Bundle args) {
        ...
        if (loaderId == VideoLibraryQueryEnum.VIDEOS.getId()) {
            ...
            LogUtils.LOGD(TAG, "Starting videos query, selection=" + selection + " (year=" + mSelectedYear
    ...
    public boolean requestModelUpdate(UserActionEnum action, @Nullable Bundle args) {
        ...
        if (action.equals(VideoLibraryUserActionEnum.VIDEO_PLAYED)) {
            if (args != null && args.containsKey(KEY_VIDEO_ID)) {
                ...
                LogUtils.LOGD(TAG, "setVideoViewed id=" + playedVideoId);
                ...
                LogUtils.LOGE(TAG, "The VideoLibraryUserActionEnum.VIDEO_VIEWED action was called without a "
                        + "proper Bundle.");
                return false;
            }
        }
